...
export async function recommend({
  ...
  // Cota de longitud: acota el costo de tokens por llamada y reduce la
  // superficie de abuso del endpoint público (cualquier visitante anónimo
  // puede invocarlo). El objetivo del anunciante cabe de sobra en 600 chars.
  const MAX_PROMPT_LENGTH = 600;
  if (cleaned.length > MAX_PROMPT_LENGTH) {
    return {
      ok: false,
      error: "El texto es demasiado largo. Resume tu objetivo en una o dos frases.",
    };
  }
