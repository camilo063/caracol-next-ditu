...
async function main() {
  // Guard de seguridad: el seed hace upsert/borrado de contenido por defecto.
  // Correrlo contra producción sobrescribiría lo editado por el cliente y
  // podría borrar Media. Bloqueado salvo override explícito.
  if (process.env.VERCEL_ENV === "production" && process.env.ALLOW_PROD_SEED !== "true") {
    throw new Error(
      "Seed bloqueado en producción. Si realmente quieres correrlo, exporta ALLOW_PROD_SEED=true.",
    );
  }
